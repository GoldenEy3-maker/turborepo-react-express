...
type OrderItemsProps = {
  ...
  price: number | "По договоренности"
  verified: boolean
  date: Date
  views: number
  clicks: number
...
const OrderItem: FC<OrderItemsProps> = ({
  clicks,
  date,
  description,
  id,
  ...
  const renderDateAgo = (date: Date) => {
    const diff = dateDiff(new Date(), date)
    ...
    <article className={styles.item}>
      <header className={styles.header}>
        ...
        <div className={styles.extraInfo}>
          <p className={styles.price}>
            {price !== "По договоренности" ? renderPrice(price) : price}
          </p>
          <span
            className={cls([styles.verified], {
              [styles._warning]: !verified,
            })}
          >
            {verified ? (
              <svg
      ...
      <footer className={styles.footer}>
        <div className={styles.info}>
          <div className={styles.infoItem}>{renderDateAgo(date)} назад</div>
