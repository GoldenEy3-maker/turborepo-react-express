...
export const Trigger: FC<Omit<DetailedHTMLProps<InputHTMLAttributes<HTMLInputElement>, HTMLInputElement>, "type">> = ({
  value,
  className,
  ...props
}) => {
  ...
  return (
    <div className={cls([className, styles.trigger])}>
      ...
      <Button
        variant="elevated"
        ref={triggerRef}
        onClick={() => setContext(state => ({ ...state, isOpen: !state.isOpen }))}
      >
        <p>{value}</p>
